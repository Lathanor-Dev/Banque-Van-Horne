...
module.exports = (req,res)=>handler(req,res, async()=>{
  ...
  const { data:user, error } = await sb.from('pret_users')
    .select('id,username,password_hash,role,role_code,is_active,protected')
  ...
  const safe = { id:user.id, username:user.username, role:user.role, role_code:user.role_code, protected:user.protected };
